...
public class EvilHangmanApp
{
  public static void main(String[] args)
  {
    ...
    do {
      ...
      */
      ...
      /*
      * Read in dictionary and store in Hashtable including only words of correct length
      * Each word in our word list serves as a key in the dictionary and is mapped to
      * a masked version of itself. The masked words are what we will compare to determine
      * word familys as we shrink our list.
      * Ex:
      * Cats -> ____
      * If the users inputs a letter (a) contained in Cats Key Value pair will be updated to
      * Cats -> _a__
      * We can count number of times each word mask appears to determine the the size of it's
      * family.
      */
      ...
      * have been made it will be n space characters, where n is the word length input
      * by the user (A blank mask). As we compare word families we will update the current mask to be
      * the largest family possible using the character entered by the user by filling in
      ...
      */
      ...
      /*
      * Game loop to run through each turn. Displays current game state and prompts user
      * for character input. User should input a single character but scanner will accept
      * a whole string and just pull out the first character.

      * After accepting input the wordMasks in the word list are updated and the word list
      * is passed to the chooseNewMask method to find all the distinct families and count
      * their members. The word family with the highest number of elements is chossen, and
  ...
  * wordMasks by checking for selected character in the word (key) and unmasking it
